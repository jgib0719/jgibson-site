#!/usr/bin/env python3
"""
CCNA Section 3 Modal Data Extractor and Remover
Extracts topicDetails JavaScript object from section3.html and removes it from the original file.
"""
...
def extract_and_remove_section3_modal_data():
    """Extract topicDetails from section3.html and remove from original file"""
    
    # File paths
    input_file = '/var/www/jgibson.site/public_html/section3.html'
    ...
    backup_file = f'{output_dir}/section3_original_backup_{timestamp}.html'
    
    print("=== CCNA Section 3 Modal Data Extraction ===")
    ...
    # Categorize topics
    categories = categorize_section3_topics(topics)
    
    # Generate output files
    output_files = {
        'json': f'{output_dir}/section3_extracted_modal_data.json',
        'markdown': f'{output_dir}/section3_extracted_modal_content.md',
        'analysis': f'{output_dir}/section3_extraction_analysis_report.md'
    }
    
    # Save JSON data
    json_data = {
        'extraction_info': {
            'source_file': 'section3.html',
    ...
    # Generate Markdown
    generate_section3_markdown(topics, categories, output_files['markdown'])
    
    # Generate analysis report
    generate_section3_analysis(topics, categories, len(full_match), output_files['analysis'])
    
    # Remove topicDetails from original file and add placeholder
    placeholder = """            // EXTRACTED: Modal data moved to quarantine folder
            // Original topicDetails object contained """ + str(len(topics)) + """ CCNA Section 3 topics
            // To restore: Run 'python3 /var/www/jgibson.site/public_html/quarantine/reinsert_section3_modal_data.py'
            // Extraction date: """ + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """
            const topicDetails = {
                // Modal content extracted and moved to quarantine folder
                // See: /var/www/jgibson.site/public_html/quarantine/section3_extracted_modal_data.json
            };"""
    ...
    print(f"🔄 Restore: python3 {output_dir}/reinsert_section3_modal_data.py")
...
def categorize_section3_topics(topics):
    """Categorize Section 3 topics (IP Connectivity)"""
    categories = {
        'DHCP Services': [],
        'NAT/PAT': [],
    ...
    category_keywords = {
        'DHCP Services': ['dhcp', 'dynamic host', 'ip helper', 'relay'],
        'NAT/PAT': ['nat', 'pat', 'network address translation', 'port address translation'],
        'Network Time Protocol': ['ntp', 'time', 'clock', 'synchronization'],
        'DNS Services': ['dns', 'domain name', 'name resolution', 'hostname'],
        'IPv6 Connectivity': ['ipv6', 'internet protocol version 6', 'dual stack'],
        'General IP Services': ['ip', 'connectivity', 'service', 'protocol']
    }
...
def generate_section3_markdown(topics, categories, output_file):
    """Generate human-readable Markdown file"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 3 - Extracted Modal Content\n\n")
...
def generate_section3_analysis(topics, categories, content_size, output_file):
    """Generate detailed analysis report"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 3 - Extraction Analysis Report\n\n")
        f.write(f"**Generated:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Source:** section3.html\n\n")
...
if __name__ == "__main__":
    extract_and_remove_section3_modal_data()