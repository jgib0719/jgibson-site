#!/usr/bin/env python3
"""
CCNA Section 1 Modal Data Extractor and Remover
Extracts topicDetails JavaScript object from section1.html and removes it from the original file.
"""
...
def extract_and_remove_section1_modal_data():
    """Extract topicDetails from section1.html and remove from original file"""
    
    # File paths
    input_file = '/var/www/jgibson.site/public_html/section1.html'
    ...
    backup_file = f'{output_dir}/section1_original_backup_{timestamp}.html'
    
    print("=== CCNA Section 1 Modal Data Extraction ===")
    ...
    # Find topicDetails object - more comprehensive pattern
    ...
    # Categorize topics
    categories = categorize_section1_topics(topics)
    
    # Generate output files
    output_files = {
        'json': f'{output_dir}/section1_extracted_modal_data.json',
        'markdown': f'{output_dir}/section1_extracted_modal_content.md',
        'analysis': f'{output_dir}/section1_extraction_analysis_report.md'
    }
    
    # Save JSON data
    json_data = {
        'extraction_info': {
            'source_file': 'section1.html',
    ...
    # Generate Markdown
    generate_section1_markdown(topics, categories, output_files['markdown'])
    
    # Generate analysis report
    generate_section1_analysis(topics, categories, len(full_match), output_files['analysis'])
    
    # Remove topicDetails from original file and add placeholder
    placeholder = """            // EXTRACTED: Modal data moved to quarantine folder
            // Original topicDetails object contained """ + str(len(topics)) + """ CCNA Section 1 topics
            // To restore: Run 'python3 /var/www/jgibson.site/public_html/quarantine/reinsert_section1_modal_data.py'
            // Extraction date: """ + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """
            const topicDetails = {
                // Modal content extracted and moved to quarantine folder
                // See: /var/www/jgibson.site/public_html/quarantine/section1_extracted_modal_data.json
            };"""
    ...
    print(f"🔄 Restore: python3 {output_dir}/reinsert_section1_modal_data.py")
...
def categorize_section1_topics(topics):
    """Categorize Section 1 topics"""
    categories = {
    ...
    category_keywords = {
        'Network Infrastructure': ['router', 'switch', 'cable', 'topology', 'infrastructure', 'hardware'],
        'Network Access': ['vlan', 'trunk', 'access', 'port', 'ethernet', 'wireless'],
        'IP Connectivity': ['routing', 'ospf', 'eigrp', 'rip', 'static', 'default'],
        'IP Services': ['dhcp', 'dns', 'nat', 'acl', 'qos', 'snmp'],
        'Security Fundamentals': ['security', 'aaa', 'authentication', 'authorization', 'vpn'],
        'Automation and Programmability': ['api', 'rest', 'automation', 'python', 'json', 'programmability'],
        'General Concepts': ['osi', 'tcp', 'ip', 'model', 'protocol', 'standard']
    }
    ...
    for topic_name, topic_data in topics.items():
        ...
        if not categorized:
            categories['General Concepts'].append(topic_name)
    
    # Remove empty categories
    return {k: v for k, v in categories.items() if v}

def generate_section1_markdown(topics, categories, output_file):
    """Generate human-readable Markdown file"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 1 - Extracted Modal Content\n\n")
...
def generate_section1_analysis(topics, categories, content_size, output_file):
    """Generate detailed analysis report"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 1 - Extraction Analysis Report\n\n")
        f.write(f"**Generated:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Source:** section1.html\n\n")
...
if __name__ == "__main__":
    extract_and_remove_section1_modal_data()