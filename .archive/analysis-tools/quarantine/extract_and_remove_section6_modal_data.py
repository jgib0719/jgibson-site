#!/usr/bin/env python3
"""
CCNA Section 6 Modal Data Extractor and Remover
Extracts topicDetails JavaScript object from section6.html and removes it from the original file.
"""
...
def extract_and_remove_section6_modal_data():
    """Extract topicDetails from section6.html and remove from original file"""
    
    # File paths
    input_file = '/var/www/jgibson.site/public_html/section6.html'
    ...
    backup_file = f'{output_dir}/section6_original_backup_{timestamp}.html'
    
    print("=== CCNA Section 6 Modal Data Extraction ===")
    ...
    # Categorize topics
    categories = categorize_section6_topics(topics)
    
    # Generate output files
    output_files = {
        'json': f'{output_dir}/section6_extracted_modal_data.json',
        'markdown': f'{output_dir}/section6_extracted_modal_content.md',
        'analysis': f'{output_dir}/section6_extraction_analysis_report.md'
    }
    
    # Save JSON data
    json_data = {
        'extraction_info': {
            'source_file': 'section6.html',
    ...
    # Generate Markdown
    generate_section6_markdown(topics, categories, output_files['markdown'])
    
    # Generate analysis report
    generate_section6_analysis(topics, categories, len(full_match), output_files['analysis'])
    
    # Remove topicDetails from original file and add placeholder
    placeholder = """            // EXTRACTED: Modal data moved to quarantine folder
            // Original topicDetails object contained """ + str(len(topics)) + """ CCNA Section 6 topics
            // To restore: Run 'python3 /var/www/jgibson.site/public_html/quarantine/reinsert_section6_modal_data.py'
            // Extraction date: """ + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + """
            const topicDetails = {
                // Modal content extracted and moved to quarantine folder
                // See: /var/www/jgibson.site/public_html/quarantine/section6_extracted_modal_data.json
            };"""
    ...
    print(f"🔄 Restore: python3 {output_dir}/reinsert_section6_modal_data.py")
...
def categorize_section6_topics(topics):
    """Categorize Section 6 topics (Wireless LANs)"""
    categories = {
        'RF Fundamentals': [],
        'Wireless Standards': [],
        'Security': [],
    ...
    category_keywords = {
        'RF Fundamentals': ['rf', 'radio frequency', 'signal', 'antenna', 'propagation', 'channel'],
        'Wireless Standards': ['802.11', 'wifi', 'standard', 'protocol', 'ieee'],
        'Security': ['security', 'wpa', 'wep', 'authentication', 'encryption', 'psk'],
        'Access Points': ['access point', 'ap', 'controller', 'lightweight', 'autonomous'],
        'Roaming and QoS': ['roaming', 'qos', 'quality', 'handoff', 'mobility'],
        'General Wireless': ['wireless', 'wlan', 'wifi', 'network']
    }
    ...
    for topic_name, topic_data in topics.items():
        ...
        if not categorized:
            categories['General Wireless'].append(topic_name)
    
    # Remove empty categories
    return {k: v for k, v in categories.items() if v}

def generate_section6_markdown(topics, categories, output_file):
    """Generate human-readable Markdown file"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 6 - Extracted Modal Content\n\n")
...
def generate_section6_analysis(topics, categories, content_size, output_file):
    """Generate detailed analysis report"""
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# CCNA Section 6 - Extraction Analysis Report\n\n")
        f.write(f"**Generated:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Source:** section6.html\n\n")
...
if __name__ == "__main__":
    extract_and_remove_section6_modal_data()